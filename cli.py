...
def front(path: str = "front"):
    ...
    typer.echo(f"Installing frontend dependencies in: {project_path}")
    subprocess.run([npm_cmd, "install"], cwd=project_path)
    typer.echo(f"Starting frontend development server in: {project_path}")
    subprocess.run([npm_cmd, "run", "dev"], cwd=project_path)
